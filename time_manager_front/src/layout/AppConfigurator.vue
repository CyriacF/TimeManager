...
onMounted(() => {
  setTimeout(() => {
    const yellow = primaryColors.value.find((c) => c.name === "yellow");
    updateColors('primary', yellow);

    const currentSurface = surfaces.value.find((s) => s.name === "neutral");
    if (currentSurface) {
      updateColors('surface', currentSurface);
    }

    // Appliquer le thème après la mise à jour des couleurs
    applyGlobalTheme();
  }, 1);
...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      <div>
        <span class="text-sm text-muted-color font-semibold">Principal</span>
